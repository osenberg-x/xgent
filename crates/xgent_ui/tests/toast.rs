...
use xgent_ui::kit::{KitPlugin, ToastMessage, ToastMarker};
use xui::i18n_bridge::Strings;
use xui_i18n::StringSource;

struct NoopStrings;
impl StringSource for NoopStrings {
    fn get(&self, key: &str, _args: &[(&str, String)]) -> String { key.to_string() }
    fn current_lang(&self) -> &str { "zh-CN" }
}
...
fn toast_lifecycle() {
    ...
    // 发 toast
    app.world_mut().write_message(ToastMessage { text: "测试提示".into() });
    app.update();

    fn count(app: &mut App) -> usize {
        let mut q = app.world_mut().query_filtered::<Entity, With<ToastMarker>>();
