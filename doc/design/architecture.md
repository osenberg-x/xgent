...
| K-08 | CodeEditor（多行/行号/undo/查找/tree-sitter 语法高亮，MVP 仅 Rust grammar） | P1（编辑器 F-11 上线） |
...
- **ToolTier**（P1 编辑器引入后）：枚举 `Read`/`Write`/`Exec`/`UiOnly`。`UiOnly` 标记只修改 UI 元数据不修改 workspace 状态的动作（打开文件/跳转/选区/滚动/切换标签），默认 `Approved` 不走确认。`resolve_policy` 对 `UiOnly` 返回 `Approved`。新增 `EditorTool`（UI-only Tier）集：`OpenFile`/`GoTo`/`SetSelection`/`ScrollTo`/`CloseTab`，agent 经此驱动编辑器动作。详见 `doc/design/editor-design.md` 6.4 与 `doc/decisions/0009-编辑器保存绕过-writefile-工具链-ui-only-tier.md`。
...
- **EditorState trait（P1 编辑器引入）**：定义在 `xgent_core`，`xgent_ui` 实现，`xgent_context` 经 trait 查询——避免 `xgent_context` 依赖 `xgent_ui`（成环）。`EditorContextProvider`（C 阶段实现）查询 `EditorState` 处理用户在对话输入框的 @ 引用（`@file`/`@cursor`/`@selection`），组装为 `ContextChunk::EditorState`。C 阶段复用 tree-sitter AST 切片喂向量库。详见 `doc/design/editor-design.md` 6.3。
...
- **D-06（已决策）**：tree-sitter grammar 分发——P1 编辑器（F-11）MVP 阶段**只内置 Rust 一种语言 grammar**，随二进制发布；不做按需下载、不做 lazy load。理由：最小可行、离线可用、避免网络/缓存层复杂度。多语言扩展需求出现时再评估（按需下载 vs lazy load vs 全内置）。详见 `doc/design/editor-design.md` 5.4。
...
| — | D-05 | 命令注册机制 | 待定 |
| — | D-06 | tree-sitter grammar 分发 | 已决策（P1 只内置 Rust，随二进制） |
| — | D-07 | 宠物等级度量 | 待定（P1） |
