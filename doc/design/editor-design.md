...
### 2.1 布局增量（F-19 终端引入后修订）

> **修订说明**：原 §2.1 "编辑器作为对话主区内可切换的标签视图" 已作废。F-19 终端引入后，编辑器/文件预览/终端统一为 `SideView`（右侧分屏）的互斥子视图，由 `SideViewContent` 四变体状态机切换。详见 `doc/design/terminal-design.md` §2.1。

现有布局：顶栏 + 文件面板（左，可折叠）+ 对话主区（右，flex:1）+ 状态栏。编辑器、文件预览、终端都是**右侧分屏 `SideView` 的互斥子视图**——`SideView` 默认收起（对话独占），展开时与对话主区并排各占一半（均 flex:1），同一时刻只显示一种子视图。

```
┌──────────────────────────────────────────────────────────┐
│ 顶栏（高 40px）                                           │
│  项目名 · provider/model 标签 · 新建会话 · 🖥终端 · ⚙设置 │
├────────────┬─────────────────────────────────────────────┤
│            │  对话主区（flex:1）                          │
│ 文件面板    │ ┌─────────────────────────────────────┐    │
│（宽 240px）│ │  消息列表 + 工具卡片 + 输入框       │    │
│ 可折叠      │ └─────────────────────────────────────┘    │
│            │┌──────────────────────────────────────┐    │
│            ││  SideView（flex:1，互斥四选一）       │    │
│            ││  · Editor（编辑器多标签）            │    │
│            ││  · Preview（文件只读预览）           │    │
│            ││  · Terminal（终端多 tab，F-19）      │    │
│            ││  · None（收起，对话独占）            │    │
│            │└──────────────────────────────────────┘    │
├────────────┴─────────────────────────────────────────────┤
│ 状态栏（高 24px）                                         │
...
`SideViewContent` 四变体（废弃原 `EditorView` 枚举，单一状态源避免两套状态打架）：

```rust
pub enum SideViewContent { None, Editor, Preview, Terminal }
```

- `SideView` 展开/收起由 `SideViewCollapsed` resource 控制；子视图显隐统一由 `apply_side_view_visibility` 系统据 `SideViewContent` 写各 Marker 节点（避免 B0001 跨系统可变访问冲突）。
- 默认 `SideViewContent = None`（收起，对话独占）。
- `Ctrl+Shift+E` 切 `Editor` + 展开；`Ctrl+Shift+D` 切 `None` + 收起；`Ctrl+\` 切 editor（已展开且 Editor 再按收起）；`Ctrl+`` 切 terminal（F-19）。
- 文件面板点击文件 → 代码文件切 `Editor` 并打开新标签；非代码文件切 `Preview`。
